...
import {
  ...
  getEndgameTrainingFeedback,
  getEndgameTrainingHint,
  recordTrainingHintLevel,
...
export default function App() {
  ...
  if (search.has('gomoku') && search.has('local'))
    return (
      <Suspense fallback={<main className="home-screen">正在载入五子棋…</main>}>
        <GomokuApp />
      </Suspense>
    )
      ...
      <section className="home-card">
        ...
        {latestGame && (
          <a className="home-continue" href={gameUrl(latestGame.id)}>
            ...
            <small>
              {formatModeName(latestGame.mode)} · {latestGame.moveCount} 回合
            </small>
            <b>继续 →</b>
          </a>
        )}
        <nav className="home-entries" aria-label="游戏入口">
          <a href="?type=xiangqi">
            <span className="home-entry-mark">象</span>
            <strong>中国象棋</strong>
            <small>普通象棋、揭棋、人机对弈、残局训练与局域网对战</small>
            <b>进入象棋 →</b>
          </a>
          <a href="?type=gomoku">
            <span className="home-entry-mark gomoku">五</span>
            <strong>五子棋</strong>
            <small>标准与有禁手规则，本地练习、AI 对决和局域网房间</small>
            <b>进入五子棋 →</b>
          </a>
        </nav>
      </section>
...
function GameModeScreen({ game }: { game: 'xiangqi' | 'gomoku' }) {
  ...
  return (
    <main className="home-screen game-console-screen">
      <section className="home-card game-console-card">
        <div className="home-brand">
          <span className={gomoku ? 'gomoku' : ''} aria-hidden="true">
            {gomoku ? '五' : '象'}
          </span>
          <div>
            <small>{gomoku ? 'GOMOKU' : 'XIANGQI'} STUDIO</small>
            <h1>{gomoku ? '五子棋' : '中国象棋'}</h1>
            <p>今天想怎么下？</p>
          </div>
        </div>
        <nav className="home-entries game-goal-entries" aria-label="棋局目标">
          <a href={gomoku ? '?gomoku=1&local=1' : '?local=1&intent=play'}>
            <span className="home-entry-mark">弈</span>
            <strong>开始一局</strong>
            <small>
              {gomoku ? '人机挑战、本地双人或观看 AI 对决' : '选择对手、规则、执子方和 AI 强度'}
            </small>
            <b>配置新对局 →</b>
          </a>
          <a href={gomoku ? '?gomoku=1&lan=1' : '?lan=1'}>
            <span className="home-entry-mark online">友</span>
            <strong>局域网对战</strong>
          ...
          <a href={gomoku ? '?gomoku=1&local=1&history=1' : '?local=1&intent=study'}>
            ...
            <strong>训练与研究</strong>
            <small>
              {gomoku ? '打开历史棋局并查看局后复盘' : '进入残局训练、研究局面与棋谱回放'}
            </small>
            <b>{gomoku ? '查看对局记录 →' : '打开训练中心 →'}</b>
          </a>
        </nav>
        <a className="home-back-link" href="?">
          ← 返回棋局空间
        </a>
      </section>
    </main>
...
function LocalApp() {
  ...
  const [studySaveStatus, setStudySaveStatus] = useState<'saved' | 'unsaved' | 'shared' | null>(
    null,
  )
      ...
      if (gameResult.status === 'fulfilled' && gameResult.value) {
        ...
        setGameStoreError(
          gameResult.reason instanceof Error
            ? `对局加载失败：${gameResult.reason.message}`
            : '对局加载失败',
        )
      }
  ...
  const game = useGame({
    ...
    initialFen:
      initialLiveState?.initialFen ||
      (gameMode === 'study' ? selectedStudy?.initialFen : selectedEndgame?.fen),
    initialMoveRecords:
      initialLiveState?.historyRecords || (gameMode === 'study' ? selectedStudy?.moves : undefined),
    initialCurrentMoveIndex:
      initialLiveState?.currentMoveIndex ??
      (gameMode === 'study' ? selectedStudy?.currentMoveIndex : undefined),
    initialAnalysisPoints: gameMode === 'study' ? selectedStudy?.analysisPoints : undefined,
    initialVariationTree:
      initialLiveState?.variationTree ||
      (gameMode === 'study' ? selectedStudy?.variationTree : undefined),
  ...
  const liveGameState = useMemo<PersistedGameState | null>(() => {
    ...
    return {
      ...
      initialJieqiBoard:
        gameMode === 'jieqi'
          ? game.initialJieqiBoard || initialLiveState?.initialJieqiBoard
          : undefined,
    ...
    game.gameStatus,
    game.gameStatusReason,
    game.historyRecords,
    game.initialFen,
    game.initialJieqiBoard,
    game.variationTree,
    gameMode,
    initialLiveState?.initialJieqiBoard,
  ])

  const handleGameSaved = useCallback((document: GameDocument) => {
    setActiveGame(document)
    setSavedGames((current) =>
      [toGameSummary(document), ...current.filter((item) => item.id !== document.id)].sort(
  ...
  useEffect(() => {
    if (!activeGame || gamePersistence.status === 'saved' || gamePersistence.status === 'idle')
      return
  ...
  const trainingFeedback = getEndgameTrainingFeedback(
    selectedEndgame,
    game.moveRecords,
    game.gameStatus,
    game.evaluation,
  )
  const trainingHint = getEndgameTrainingHint(
    selectedEndgame,
    game.moveRecords,
    game.board,
    trainingHintLevel,
  ...
  const candidatePreviewFrame = useMemo(
    () =>
      candidatePreview
        ? getCandidatePreviewFrame(
            candidateAutoPositionKey,
            candidatePreview.records,
            candidatePreview.stepIndex,
          )
        : null,
  ...
  const studyContentSignature = useMemo(
    () => createStudyContentSignature(studyContent),
    [studyContent],
  )
      ...
      steps: [
        {
          mark: '走',
          title: '按原位走法',
          description: '暗子先按所在位置对应的棋种移动。',
        },
        {
          mark: '翻',
          title: '落子后揭晓',
          description: '暗子完成移动后，公开真实身份。',
        },
        {
          mark: '隐',
          title: '暗吃信息私有',
          description: '被暗吃棋子的身份只对捕获方可见。',
        },
      ],
        ...
        <StartScreen
          ...
          onRename={async (summary, name) => {
            try {
              ...
              setSavedGames((current) =>
                current.map((item) => (item.id === renamed.id ? toGameSummary(renamed) : item)),
              )
            } catch (error) {
  ...
  if (gameMode === 'endgame' && !selectedEndgame) {
    return (
      <EndgameLibrary
        ...
        onExportJson={() =>
          downloadJson('xiangqi-custom-endgames.json', exportCustomEndgamesJson())
        }
        onImportJson={(file) => {
          file
            .text()
  ...
  if (gameMode === 'study' && !selectedStudy) {
    return (
      <StudyLibrary
        ...
        onExportJson={() =>
          downloadJson('xiangqi-study-positions.json', exportStudyPositionsJson())
        }
        onImportJson={(file) => {
          file
            .text()
      ...
      <header className="workspace-header">
        ...
        <div className="workspace-header-actions">
          <div className="workspace-context">
            ...
            {gameMode === 'endgame' && selectedEndgame?.name && (
              <strong>{selectedEndgame.name}</strong>
            )}
            {gameMode === 'study' && selectedStudy?.name && <strong>{selectedStudy.name}</strong>}
            <span className={`workspace-turn ${game.currentTurn}`}>
              {game.gameStatus === 'playing'
                ? `${game.currentTurn === 'red' ? '红方' : '黑方'}走棋`
                : '棋局结束'}
            </span>
          </div>
          <span
            className={`workspace-save-state ${activeGame ? gamePersistence.status : studySaveStatus || 'saved'}`}
          >
            {activeGame
              ? gamePersistence.status === 'saved' || gamePersistence.status === 'idle'
                ? '已保存'
                : formatSaveStatus(gamePersistence.status, gamePersistence.error)
              : gameMode === 'study' && studySaveStatus === 'unsaved'
                ? '等待自动保存'
      ...
      <div className="game-container">
        ...
        <div className="board-stage">
          {activeGame && game.leaseStatus !== 'granted' && (
            <div className="game-access-banner">
              <span>
                {game.leaseStatus === 'requesting'
                  ? '正在获取编辑权…'
                  : '此对局已在另一个页面编辑，当前为只读模式。'}
              </span>
              {game.leaseStatus === 'readonly' && (
                <button onClick={game.takeoverGame}>接管编辑</button>
              )}
            </div>
          )}
          ...
          {gameMode === 'study' && !selectedTrainingTask && annotationTool && (
            <div className="board-annotation-mode" role="status">
              <span>
                ...
                {annotationTool === 'arrow' ? '箭头' : '圈点'}标注中
              </span>
          ...
          {candidatePreview && candidatePreviewFrame && (
            <CandidatePreviewControls
              candidateLabel={
                candidatePreview.candidate.notation || candidatePreview.candidate.move
              }
        ...
        <div className={`side-panel ${mobileToolsOpen ? 'mobile-tools-open' : ''}`}>
          <nav className="workspace-tabs" aria-label="棋局工具">
            ...
            {showReviewTab && (
              <button
                className={workspaceTab === 'review' ? 'active' : ''}
                onClick={() => selectWorkspaceTab('review')}
              >
                复盘
                {moveReviews.length > 0 ? <span>{moveReviews.length}</span> : null}
              </button>
            )}
            {showVariationsTab && (
              <button
                className={workspaceTab === 'variations' ? 'active' : ''}
                onClick={() => selectWorkspaceTab('variations')}
              >
                变招
            ...
            {!selectedTrainingTask && gameMode === 'study' && (
              ...
              >
                标注
                {game.currentNodeAnnotations.length > 0 ? (
                  <span>{game.currentNodeAnnotations.length}</span>
                ) : null}
              </button>
            )}
          </nav>
          <div className="workspace-tool-content">
            ...
            {workspaceTab === 'play' && (
              <GamePanel
                ...
                canJumpToPrevMarked={
                  gameMode === 'study' &&
                  findPrevMarkedIndex(game.historyRecords, game.currentMoveIndex) !== null
                }
                canJumpToNextMarked={
                  gameMode === 'study' &&
                  findNextMarkedIndex(game.historyRecords, game.currentMoveIndex) !== null
                }
                ...
                onSaveAsEndgame={() => {
                  setEditorDraft({
                    ...
                    description:
                      gameMode === 'endgame' && selectedEndgame
                        ? `基于残局「${selectedEndgame.name}」另存`
                        : '从当前局面保存',
                    fen: game.getCurrentFen(),
                    tags:
                      gameMode === 'endgame' && selectedEndgame ? selectedEndgame.tags || [] : [],
                  ...
                  const values = await requestProductDialog({
                    ...
                    fields: [
                      ...
                      },
                      {
                        name: 'description',
                        label: '研究说明',
                        initialValue: selectedStudy?.description || '',
                        multiline: true,
                        maxLength: 300,
                      },
                    ],
                ...
                onTrainingHint={() => {
                  ...
                  const hint = getEndgameTrainingHint(
                    selectedEndgame,
                    game.moveRecords,
                    game.board,
                    nextLevel,
                  ...
                  setTrainingHintHistory((history) =>
                    recordTrainingHintLevel(
                      history,
                      game.currentMoveIndex,
                      nextLevel,
                ...
                onResign={async () => {
                  const confirmed = await requestProductDialog({
                    title: '确认认输',
                    description: `当前${game.currentTurn === 'red' ? '红方' : '黑方'}将认输并结束棋局。`,
                    confirmLabel: '确认认输',
                    dangerous: true,
                  })
            ...
            {gameMode !== 'jieqi' && workspaceTab === 'engine' && (
              <CandidateList
                ...
                onPreview={(candidate) => {
                  try {
                    ...
                    setCandidatePreview({
                      candidate,
                      records,
                      stepIndex: Math.min(1, records.length),
                    })
                  } catch {
                    void requestProductDialog({
                      title: '无法预览候选变化',
                      description: '当前引擎返回的变化无法从这个局面复现。',
                      confirmLabel: '知道了',
                      cancelLabel: null,
                    })
                  }
            ...
            {gameMode !== 'jieqi' && workspaceTab === 'engine' && showAnalysis && (
              <AnalysisCurve points={game.analysisPoints} />
            )}
          </div>
        </div>
      </div>
...
function formatSaveStatus(status: GameSaveStatus, error: string): string {
  ...
  if (status === 'conflict')
    return `保存冲突，请返回对局列表后重新打开。${error ? ` ${error}` : ''}`
  if (status === 'error') return `保存失败，将保留当前页面内容。${error ? ` ${error}` : ''}`
  return ''
}

function scenarioNameFromState(
  gameMode: GameMode,
  selectedEndgame: EndgameDefinition | null,
): string {
  if (gameMode === 'endgame' && selectedEndgame) {
...
function FenDialog({
  ...
  const validation = mode === 'import' && value.trim() ? validateFenPosition(value) : null
    ...
    <div className="start-screen">
      <div className="start-card">
        <section className="start-hero">
          <div className="start-seal" aria-hidden="true">
            弈
          </div>
          ...
          <a className="start-home-link" href="?type=xiangqi">
            ← 返回象棋控制台
          </a>
        </section>
        ...
        <section className="start-config">
          ...
          <div className="start-section-switch" aria-label="象棋目标">
            <button
              className={section === 'play' ? 'active' : ''}
              onClick={() => setSection('play')}
            >
              <strong>开始一局</strong>
              <span>人机、本地双人或 AI 对决</span>
            </button>
            <button
              className={section === 'study' ? 'active' : ''}
              onClick={() => setSection('study')}
            >
              <strong>训练与研究</strong>
              <span>残局训练、研究局面与回放</span>
            </button>
          </div>
          ...
          {section === 'play' ? (
            <>
              <div className="option-group mode-option-group">
                ...
                <div className="btn-group opponent-btn-group">
                  <button
                    className={opponent === 'human-vs-ai' ? 'active' : ''}
                    onClick={() => setOpponent('human-vs-ai')}
                  >
                    挑战 AI
                  </button>
                  <button
                    className={opponent === 'human-vs-human' ? 'active' : ''}
                    onClick={() => {
                      setOpponent('human-vs-human')
                      setRule('xiangqi')
                    }}
                  >
                    本地双人
                  ...
                  <button
                    className={opponent === 'ai-vs-ai' ? 'active' : ''}
                    onClick={() => {
                      setOpponent('ai-vs-ai')
                      setRule('xiangqi')
                    }}
                  >
                    AI 对决
              ...
              <div className="option-group rule-option-group">
                ...
                <div className="rule-choice-grid">
                  <button
                    className={rule === 'xiangqi' ? 'active' : ''}
                    onClick={() => setRule('xiangqi')}
                  >
                    <strong>普通象棋</strong>
                    <span>完整支持全部对手类型和分析工具</span>
                  </button>
                  <button
                    className={rule === 'jieqi' ? 'active' : ''}
                    onClick={() => {
                      setRule('jieqi')
                      setOpponent('human-vs-ai')
                    }}
                  >
                    <strong>揭棋</strong>
                    <span>暗子移动后揭晓，当前支持人机对弈</span>
                  </button>
                </div>
              </div>
            </>
            ...
            <div className="option-group study-choice-group">
              ...
              <div className="study-choice-grid">
                <button
                  className={studyMode === 'endgame' ? 'active' : ''}
                  onClick={() => setStudyMode('endgame')}
                >
                  <strong>残局训练</strong>
                  <span>从内置或自定义残局开始，使用分层提示练习</span>
                </button>
                <button
            ...
            <>
              ...
              <div className="option-group">
                ...
                <div className="btn-group">
                  <button
                    className={diff === 'easy' ? 'active' : ''}
                    onClick={() => setDiff('easy')}
                  >
                    初级 · D8
                  </button>
                  <button
                    className={diff === 'medium' ? 'active' : ''}
                    onClick={() => setDiff('medium')}
                  >
                    中级 · D14
                  </button>
                  <button
                    className={diff === 'hard' ? 'active' : ''}
                    onClick={() => setDiff('hard')}
                  >
                    高级 · D20
                  </button>
                  <button
                    className={diff === 'master' ? 'active' : ''}
                    onClick={() => setDiff('master')}
                  >
                    大师 · D26
                  </button>
                </div>
              ...
              <div className="option-group">
                ...
                <div className="btn-group">
                  <button
                    className={side === 'red' ? 'active red-side' : ''}
                    onClick={() => setSide('red')}
                  >
                    执红先行
                  </button>
                  <button
                    className={side === 'black' ? 'active black-side' : ''}
          ...
          {mode === 'ai-vs-ai' && (
            <>
              <div className="option-group">
                ...
                <div className="btn-group">
                  <button
                    className={redDiff === 'easy' ? 'active' : ''}
                    onClick={() => setRedDiff('easy')}
                  >
                    初级 · D8
                  </button>
                  <button
                    className={redDiff === 'medium' ? 'active' : ''}
                    onClick={() => setRedDiff('medium')}
                  >
                    中级 · D14
                  </button>
                  <button
                    className={redDiff === 'hard' ? 'active' : ''}
                    onClick={() => setRedDiff('hard')}
                  >
                    高级 · D20
                  </button>
                  <button
                    className={redDiff === 'master' ? 'active' : ''}
                    onClick={() => setRedDiff('master')}
                  >
                    大师 · D26
                  </button>
                </div>
              ...
              <div className="option-group">
                ...
                <div className="btn-group">
                  <button
                    className={blackDiff === 'easy' ? 'active' : ''}
                    onClick={() => setBlackDiff('easy')}
                  >
                    初级 · D8
                  </button>
                  <button
                    className={blackDiff === 'medium' ? 'active' : ''}
                    onClick={() => setBlackDiff('medium')}
                  >
                    中级 · D14
                  </button>
                  <button
                    className={blackDiff === 'hard' ? 'active' : ''}
                    onClick={() => setBlackDiff('hard')}
                  >
                    高级 · D20
                  </button>
                  <button
                    className={blackDiff === 'master' ? 'active' : ''}
                    onClick={() => setBlackDiff('master')}
                  >
                    大师 · D26
                  </button>
                </div>
          ...
          <div className="saved-games-section">
            ...
            {loading ? (
              <ProductState kind="loading" title="正在读取对局" />
            ) : games.length === 0 ? (
              <ProductState
                title="还没有保存的对局"
                description="完成或开始一盘本地对局后，会自动显示在这里。"
              />
            ) : (
                ...
                {games.slice(0, 8).map((item) => (
                  <div className="saved-game-item" key={item.id}>
                    <button className="saved-game-open" onClick={() => onOpen(item.id)}>
                      ...
                      <span>
                        {formatModeName(item.mode)} · {item.moveCount} 回合 ·{' '}

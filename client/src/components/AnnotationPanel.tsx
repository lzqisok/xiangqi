  ...
  return (
    <section className="annotation-panel">
      ...
      <div className="annotation-colors" aria-label="标注颜色">
        {(['red', 'green', 'blue'] as const).map((item) => (
          <button
            key={item}
      ...
      <p className="annotation-help">
        选择工具后，在棋盘上拖动绘制箭头，或点按棋子位置添加圈点。再次点击当前工具可退出。
      </p>
      <div className="annotation-actions">
        <button onClick={onUndo} disabled={annotations.length === 0}>
          撤销最近
        </button>
        <button onClick={onClear} disabled={annotations.length === 0}>
          清空当前节点
        </button>
      </div>
      ...
      {annotations.length > 0 ? (
        <ol className="annotation-list">
          {annotations.map((annotation) => (
            <li key={annotation.id}>
              ...
              <button
                aria-label={`删除${formatAnnotation(annotation)}`}
        ...
        <p className="annotation-empty">当前节点还没有标注。</p>
      )}
    </section>
  )
}

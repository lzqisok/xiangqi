  ...
  return (
    <div className="candidate-list">
      <div className="candidate-header">
        <h3>候选走法</h3>
        <button onClick={onRequest} disabled={!canRequest}>
          {thinking ? '计算中...' : '刷新'}
        </button>
      </div>
      <div className="candidate-quick-settings">
        ...
        <span>
          {candidateCount} 路 ·{' '}
          {searchMode === 'depth' ? `D${searchDepth}` : `${searchTimeMs / 1000}s`}
        </span>
      </div>
      <details className="engine-settings-disclosure">
        ...
        <div className="engine-settings-panel">
          ...
          <label>
            刷新间隔
            <select
              value={autoRefreshDelay}
              onChange={(e) => onAutoRefreshDelayChange(Number(e.target.value))}
            >
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) => onHintDifficultyChange(e.target.value as Difficulty)}
            >
          ...
          <label>
            ...
            <select
              value={searchMode}
              onChange={(e) => onSearchModeChange(e.target.value as EngineSearchMode)}
            >
          ...
          {searchMode === 'depth' ? (
            ...
            <label>
              ...
              <select
                value={searchTimeMs}
                onChange={(e) => onSearchTimeMsChange(Number(e.target.value))}
              >
                <option value={500}>0.5 秒</option>
          ...
          <label>
            ...
            <select
              value={engineThreads}
              onChange={(e) =>
                onEngineThreadsChange(e.target.value === 'auto' ? 'auto' : Number(e.target.value))
              }
            >
              <option value="auto">自动</option>
              {[1, 2, 3, 4, 5, 6, 7, 8].map((threads) => (
                <option value={threads} key={threads}>
                  {threads} 线程
                </option>
              ))}
            </select>
          ...
          <label>
            ...
            <select
              value={engineHashMb}
              onChange={(e) => onEngineHashMbChange(Number(e.target.value))}
            >
              {[16, 32, 64, 128, 256, 512].map((hashMb) => (
                <option value={hashMb} key={hashMb}>
                  {hashMb} MB
                </option>
              ))}
            </select>
        ...
        candidates.map((candidate, index) => (
          ...
          >
            ...
            {candidate.pvNotation && candidate.pvNotation.length > 1 && (
              <div className="candidate-pv">{candidate.pvNotation.slice(1).join('  ')}</div>
            )}

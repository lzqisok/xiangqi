...
export default function ProductDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div className={`product-dialog-mark ${dangerous ? 'dangerous' : ''}`} aria-hidden="true">
          {dangerous ? '慎' : '弈'}
        </div>
        <h2 id="product-dialog-title">{title}</h2>
        {description && <p>{description}</p>}
        {steps.length > 0 && (
          <div className="product-dialog-steps">
            {steps.map((step) => (
              <div key={step.title}>
        ...
        {fields.length > 0 && (
          <div className="product-dialog-fields">
        ...
        <div className="product-dialog-actions">
          ...
          <button
            type="submit"
            className={dangerous ? 'dangerous' : 'primary'}
            disabled={!canConfirm}
          >
            {confirmLabel}
          </button>

...
function historySignature(moves: Array<{ row: number; col: number; player: number }>) {
  return moves.map((move) => `${move.row},${move.col},${move.player}`).join(';')
}
  ...
  return (
    <div className="gomoku-page">
      <header className="gomoku-header">
        ...
        <div className="gomoku-header-actions">
          <button
            className="gomoku-home-link"
            onClick={() => {
          ...
          <a href="?type=gomoku" className="gomoku-home-link">
            返回模式选择
          </a>
        </div>
      </header>
      ...
      <main className="gomoku-workspace">
              ...
              {!isStarted && (
                <div className="gomoku-start-overlay">
                  <div>
                    ...
                    <button className="gomoku-btn-primary" onClick={newGame}>
                      开始游戏
                    </button>
                  </div>
                </div>
        ...
        <aside
          className={`gomoku-side-panels mobile-stage-${mobileStage} ${mobilePanelOpen ? 'mobile-panel-open' : ''}`}
          aria-label="五子棋阶段工具"
        >
          <div className="gomoku-panel-slot status">
            <GameStatus
              isStarted={isStarted}
              canUndo={canUndo}
              onUndo={undo}
          ...
          <div className="gomoku-panel-slot settings">
            <GameControls />
          </div>
          <div className="gomoku-panel-slot review">
            <ReviewPanel />
          </div>
          <button
            className="gomoku-mobile-panel-close"
            aria-label="收起阶段工具"
      ...
      {showRestartConfirm && (
        <ProductDialog
          title="重开一局"
          description="当前对局进度会被清空，确定重新开始吗？"
          confirmLabel="确定重开"
          dangerous
      ...
      {showHistory && (
        <div
          className="gomoku-modal-overlay"
          role="presentation"
          onMouseDown={() => setShowHistory(false)}
        >
          <section
            className="gomoku-modal gomoku-history-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="gomoku-history-title"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <h2 id="gomoku-history-title">本地对局记录</h2>
            {history.length === 0 ? (
              <ProductState
                title="还没有已结束的对局"
                description="完成一局后，对局记录会自动保存在这里。"
              />
            ) : (
              <div className="gomoku-history-list">
                {history.map((record) => (
                  <button
                    key={record.id}
                    onClick={() => {
                      savedSignature.current = historySignature(record.moves)
                      loadRecordedGame(record.moves, record.forbiddenEnabled)
                      setShowHistory(false)
                    }}
                  >
                    <strong>
                      {record.draw ? '和棋' : `${record.winner === 1 ? '黑方' : '白方'}获胜`}
                    </strong>
                    <span>
                      {record.mode === 'pvp'
                        ? '本地双人'
                        : record.mode === 'ai'
                          ? '人机对战'
                          : 'AI 对决'}{' '}
                      · {record.forbiddenEnabled ? '有禁手' : '无禁手'} · {record.moves.length} 手
                    </span>
            ...
            <div>
              ...
              <button className="gomoku-btn-primary" onClick={() => setShowHistory(false)}>
                关闭
              </button>
            </div>
          </section>
      ...
      {showClearHistoryConfirm && (
        <ProductDialog
          title="清空对局记录"
          description="所有本地五子棋对局记录都会被删除，且无法恢复。"
          confirmLabel="确认清空"
          dangerous

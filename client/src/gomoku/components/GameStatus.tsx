...
export function GameStatus({ isStarted, canUndo, onUndo, onRestart }: GameStatusProps) {
  ...
  return (
    <section className="gomoku-panel gomoku-status-panel">
      <h3>对局状态</h3>
      <p className="gomoku-status-main">
        {winner ? `${winnerText}获胜` : draw ? '平局' : `${turnText}行棋`}
      </p>
      ...
      <p className="gomoku-ai-match-meta">
        ...
        {mode === 'ai-vs-ai' && (
          <>
            ...
            <span>·</span>白 {GOMOKU_DIFFICULTY_LABELS[whiteAiDifficulty]}
          </>
        )}
      </p>
      {isStarted && (
        <div className="gomoku-status-actions">
          <button
            className="gomoku-btn-secondary"
            disabled={aiThinking || !canUndo}
            onClick={onUndo}
          >
            悔棋
          </button>
          <button className="gomoku-btn-primary" onClick={onRestart}>
            重开一局
          </button>
        </div>
      )}
      {isStarted && mode === 'ai-vs-ai' && (
        <div className="gomoku-ai-match-actions">
          <button
            className="gomoku-btn-secondary"
            disabled={aiThinking || winner !== null || draw}
            onClick={nextAiMove}
          >
            {aiThinking ? '思考中…' : '下一步'}
          </button>
          <button
            className="gomoku-btn-primary"
          ...
          <select
            aria-label="AI 对战速度"
            value={aiAutoDelay}
            onChange={(event) => setAiAutoDelay(Number(event.target.value))}
          >

...
function ReviewContent({ review, step, reviewCursor, setReviewCursor, expanded = false }: ReviewContentProps) {
  return (
    <div className={`gomoku-review-content ${expanded ? 'gomoku-review-content--expanded' : ''}`}>
      <div className="gomoku-review-nav">
        <button className="gomoku-btn-secondary" disabled={reviewCursor <= 0} onClick={() => setReviewCursor(reviewCursor - 1)}>
          上一步
        </button>
        <span>第 {step.ply} 手 / 共 {review.steps.length} 手</span>
        <button
      ...
      <div className="gomoku-review-details">
        <div className="gomoku-review-step">
          <strong>本手评价</strong>
          <p>实战着法：{step.player === BLACK ? '黑' : '白'}方（{step.playedMove.row + 1}，{step.playedMove.col + 1}）</p>
          <p>评估偏差：{Math.round(step.delta)}（{gradeText(step.grade)}）</p>
          <p>黑方局势值：{Math.round(step.evalBlackBefore)} → {Math.round(step.evalBlackAfter)}</p>
        </div>
        ...
        <div className="gomoku-review-summary">
          <strong>全局总结</strong>
          <p>关键转折手：{review.summary.keyTurns.length ? review.summary.keyTurns.join('、') : '无'}</p>
      ...
      <section className="gomoku-panel gomoku-review-panel">
        <div className="gomoku-review-header">
          ...
          <div className="gomoku-review-header-actions">
            {reviewLoading && (
              <button className="gomoku-btn-secondary" onClick={stopReviewAnalysis}>停止</button>
            )}
            {review && step && (
              <button className="gomoku-btn-primary" onClick={() => setExpanded(true)}>完整查看</button>
            )}
            <button className="gomoku-btn-secondary" disabled={reviewLoading || moveHistory.length === 0} onClick={runReviewAnalysis}>
              {reviewLoading && reviewProgress
                ? `分析 ${reviewProgress.completed}/${reviewProgress.total}`
                : reviewLoading ? '分析中...' : '重新分析'}
            </button>
          </div>
        </div>
        ...
        {!review && !reviewLoading && (
          <p className="gomoku-review-empty">对局结束后会自动生成复盘；你也可以手动点击“重新分析”。</p>
        )}
        ...
        {reviewLoading && (
          <div className="gomoku-review-progress" aria-live="polite">
            <div>
              <span>正在逐手分析</span>
              <strong>{reviewProgress ? `${reviewProgress.completed}/${reviewProgress.total}` : '准备中'}</strong>
            </div>
        ...
        {review && step && (
          <ReviewContent review={review} step={step} reviewCursor={reviewCursor} setReviewCursor={setReviewCursor} />
        )}
      </section>

      {expanded && review && step && (
        <div className="gomoku-review-overlay" role="presentation" onMouseDown={() => setExpanded(false)}>
          <section
          ...
          >
            <header>
              ...
              <button className="gomoku-btn-secondary" onClick={() => setExpanded(false)}>关闭</button>
            </header>
            <ReviewContent
              review={review}
              step={step}

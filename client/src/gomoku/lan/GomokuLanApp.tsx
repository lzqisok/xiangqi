...
function Lobby({ nickname, onNickname }: { nickname: string; onNickname: (value: string) => void }) {
  ...
  return <div className="lan-shell gomoku-lan-shell"><header className="lan-header"><div><h1>五子棋局域网对战</h1><p>同一局域网内创建房间、邀请棋友、聊天或观战</p></div><a href="?type=gomoku">返回模式选择</a></header>
    ...
    {error && <div className="lan-error">{error}</div>}{lobbyView === 'find' && <section className="lan-lobby card"><div className="lan-lobby-heading"><div><h2>房间列表</h2><p>公开、最近进入和最近结束统一展示</p></div><div className="lan-room-filters">{(['all', 'waiting', 'playing', 'finished', 'recent'] as const).map(filter => <button className={roomFilter === filter ? 'active' : ''} key={filter} onClick={() => setRoomFilter(filter)}>{{ all: '全部', waiting: '可加入', playing: '进行中', finished: '已结束', recent: '最近进入' }[filter]}</button>)}</div></div>{roomItems.length === 0 && recentOnly.length === 0 ? <p className="lan-empty">当前筛选下没有房间。</p> : <div className="lan-room-list">{roomItems.map(({ room, recent: isRecent }) => <button className="lan-room-card" key={room.id} onClick={() => enter(room.id)}><strong>{room.name}</strong><span>{room.gomokuRule === 'renju' ? '黑方禁手' : '标准五子棋'} · {room.phase === 'waiting' ? '可加入' : room.phase === 'playing' ? '进行中' : resultText(room.status, room.statusReason)}{isRecent ? ' · 最近进入' : ''}</span><span>黑：{room.red || '空席'}　白：{room.black || '空席'}　{room.phase === 'finished' ? `共 ${room.moveCount} 手` : `观众 ${room.spectatorCount}`}</span></button>)}{recentOnly.map(item => <button className="lan-room-card" key={item.id} onClick={() => enter(item.id)}><strong>{item.name}</strong><span>五子棋 · 最近进入</span><span>{item.role === 'red' ? '黑方席位' : item.role === 'black' ? '白方席位' : item.role === 'owner' ? '房主' : '观众'}</span></button>)}</div>}</section>}
  </div>
}
    ...
    <main className="lan-ready-layout">
      <section className="lan-ready-room card">
        ...
        <div className="lan-ready-seats">{(['red', 'black'] as const).map(side => {
          ...
          return <article className={`lan-ready-seat gomoku ${side} ${current ? 'current' : ''}`} key={side}><div><i>{side === 'red' ? '●' : '○'}</i><span><strong>{sideName(side)} · {side === 'red' ? '先手' : '后手'}</strong><small>{seat ? `${seat.nickname} · ${seat.online ? '在线' : '离线'} · ${seat.ready ? '已准备' : '未准备'}` : '席位空缺'}</small></span></div><button className={current && seat?.ready ? 'ready' : ''} disabled={disabled} onClick={() => act(side)}>{seatActionText(side)}</button>{room.isOwner && seat && !current && <button className="remove" disabled={pending} onClick={() => send('room-remove-seat', { side })}>移除成员</button>}</article>
        })}</div>

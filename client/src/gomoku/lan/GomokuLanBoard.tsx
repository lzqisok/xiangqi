...
export function GomokuLanBoard({
  board,
  moves,
  disabled,
  showOrder,
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < 15; index++) {
      ...
      ctx.lineTo(SIZE - PAD, offset)
      ctx.stroke()
      ctx.beginPath()
      ctx.moveTo(offset, PAD)
      ctx.lineTo(offset, SIZE - PAD)
      ctx.stroke()
    }
    ctx.fillStyle = '#5a3919'
    for (const [row, col] of STARS) {
      ctx.beginPath()
      ctx.arc(PAD + col * CELL, PAD + row * CELL, 3, 0, Math.PI * 2)
      ctx.fill()
    }
    for (let row = 0; row < 15; row++)
      for (let col = 0; col < 15; col++) {
        ...
        ctx.fill()
        if (showOrder) {
          ctx.fillStyle = stone === 'red' ? '#fff' : '#333'
          ctx.font = '700 11px sans-serif'
          ctx.textAlign = 'center'
          ctx.textBaseline = 'middle'
          ctx.fillText(String(order.get(`${row},${col}`) || ''), x, y)
        }
      }
    ...
    if (winner && lastMove) {
      for (const [dr, dc] of [
        ...
        for (
          let row = lastMove.row + dr, col = lastMove.col + dc;
        ...
        ctx.lineCap = 'round'
        ctx.beginPath()
        ctx.moveTo(PAD + start.col * CELL, PAD + start.row * CELL)
        ctx.lineTo(PAD + end.col * CELL, PAD + end.row * CELL)
        ctx.stroke()
        break
      }
    }
  ...
  return (
    <canvas
      ref={ref}
      className="gomoku-lan-board"
      aria-label="五子棋棋盘"
      onClick={(event) => {
        if (disabled) return
        const rect = event.currentTarget.getBoundingClientRect(),
          border = event.currentTarget.clientLeft

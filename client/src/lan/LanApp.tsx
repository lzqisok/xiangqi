...
import { createLanRoom, listLanRooms } from './api'
import { copyLanText } from './browser'
import { LanRoomSummary } from './types'
import { getLanRecentRooms, saveLanToken, useLanRoom } from './useLanRoom'
...
export default function LanApp() {
  ...
  if (roomId) return <LanRoom roomId={roomId} nickname={nickname || '访客'} onNickname={value => { setNickname(value); localStorage.setItem(NICKNAME_KEY, value) }} />
  return <LanLobby nickname={nickname} onNickname={value => { setNickname(value); localStorage.setItem(NICKNAME_KEY, value) }} />
}
...
function LanLobby({ nickname, onNickname }: { nickname: string; onNickname: (value: string) => void }) {
  ...
  useEffect(() => {
    let active = true
    const refresh = () => listLanRooms().then(value => active && setRooms(value)).catch(cause => active && setError(cause.message))
  ...
  const create = async () => {
    ...
    try {
      ...
      localStorage.setItem(`xiangqi-lan-invite:${result.room.id}`, result.inviteToken)
...
function LanRoom({ roomId, nickname, onNickname }: { roomId: string; nickname: string; onNickname: (value: string) => void }) {
  const { room, connected, error, privateHint, setPrivateHint, send } = useLanRoom(roomId, nickname)
  ...
  const shareToken = localStorage.getItem(`xiangqi-lan-invite:${roomId}`) || ''
  ...
  return <div className="lan-shell">
    ...
    <div className="lan-game">
      <main className="board-stage"><Board board={room.board} gameStatus={room.status} gameStatusReason={mapReason} selectedPos={selected} legalMoves={legal} lastMove={lastMove} hintMove={hintMove} inCheck={room.phase === 'playing' && isInCheck(room.board, room.turn, room.variant) ? findKing(room.board, room.turn) : null} flipped={color === 'black'} aiThinking={false} thinkingText="" interactionDisabled={!canMove} onCellClick={click} onCancelSelection={() => setSelected(null)} />
      ...
      {room.phase === 'waiting' ? <aside className="lan-panel lan-room-waiting card">
        ...
        <div className="lan-seats">{(['red', 'black'] as const).map(side => <div className={`lan-seat ${side}`} key={side}><h3>{side === 'red' ? '红方' : '黑方'}</h3>{room.seats[side] ? <><strong>{room.seats[side]!.nickname}</strong><span>{room.seats[side]!.online ? '在线' : '离线'} · {room.seats[side]!.ready ? '已准备' : '未准备'}</span>{color === side && <button className="lan-ready-button" onClick={() => send('room-ready', { ready: !room.seats[side]!.ready })}>{room.seats[side]!.ready ? '取消准备' : '准备开局'}</button>}</> : <button disabled={!nickname.trim()} onClick={() => actionForSeat(side)}>{incomingInvite || room.isOwner ? `成为${side === 'red' ? '红方' : '黑方'}` : `申请${side === 'red' ? '红方' : '黑方'}`}</button>}</div>)}</div>
        {color && room.seats.red && room.seats.black && <button onClick={() => send('room-swap-request')}>申请交换红黑</button>}
        {room.pendingSwapBy && room.pendingSwapBy !== color && color && <div className="lan-request">对方申请交换红黑 <button onClick={() => send('room-swap-response', { accept: true })}>同意</button><button onClick={() => send('room-swap-response', { accept: false })}>拒绝</button></div>}
        {room.isOwner && shareToken && (!room.seats.red || !room.seats.black) && <><button onClick={shareInvite}>{copyState === 'copied' ? '邀请链接已复制' : '复制玩家邀请链接'}</button>{copyState === 'manual' && <label className="lan-copy-fallback">浏览器未允许自动复制，请手动复制<input readOnly value={inviteUrl} onFocus={event => event.currentTarget.select()} /></label>}</>}
        ...
        {color && room.phase === 'playing' && <><button disabled={room.turn !== color || (room.seats[color]?.hintsRemaining || 0) < 1} onClick={() => send('room-hint')}>大师提示（剩余 {room.seats[color]?.hintsRemaining || 0}）</button>{room.variant === 'xiangqi' && <button onClick={() => send('room-undo-request')}>申请悔棋</button>}<button onClick={() => send('room-draw-offer')}>提议和棋</button><button onClick={() => send('room-resign')}>认输</button></>}
        {room.pendingUndoBy && room.pendingUndoBy !== color && color && <Request text="对方申请悔棋" accept={() => send('room-undo-response', { accept: true })} reject={() => send('room-undo-response', { accept: false })} />}
        {room.pendingDrawBy && room.pendingDrawBy !== color && color && <Request text="对方提议和棋" accept={() => send('room-draw-response', { accept: true })} reject={() => send('room-draw-response', { accept: false })} />}
        <h3>走棋记录</h3><div className="lan-moves">{room.moves.map((move, index) => <span key={index}>{index + 1}. {move.uci}{move.revealed ? ` 揭${move.revealed}` : ''}{move.capturedHidden ? ` 吃${move.captured || '暗子'}` : move.captured ? ` 吃${move.captured}` : ''}</span>)}</div>
      </aside>}
    </div>
  </div>
}

function Request({ text, accept, reject }: { text: string; accept: () => void; reject: () => void }) { return <div className="lan-request">{text}<button onClick={accept}>同意</button><button onClick={reject}>拒绝</button></div> }
...
function formatResult(status: string, reason?: string) { if (status === 'draw') return '和棋'; return `${status === 'red-wins' ? '红方胜' : '黑方胜'}${reason === 'disconnect' ? '（对方断线）' : reason === 'resignation' ? '（对方认输）' : ''}` }

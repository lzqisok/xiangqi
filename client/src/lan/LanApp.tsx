    ...
    <main className="lan-ready-layout">
      <section className="lan-ready-room card">
        ...
        <div className="lan-ready-seats">{(['red', 'black'] as const).map(side => {
          ...
          return <article className={`lan-ready-seat ${side} ${current ? 'current' : ''}`} key={side}><div><i>{side === 'red' ? '红' : '黑'}</i><span><strong>{side === 'red' ? '红方 · 先行' : '黑方 · 后手'}</strong><small>{seat ? `${seat.nickname} · ${seat.online ? '在线' : '离线'} · ${seat.ready ? '已准备' : '未准备'}` : '席位空缺'}</small></span></div><button className={current && seat?.ready ? 'ready' : ''} disabled={disabled} onClick={() => actOnBoardSeat(side)}>{boardSeatLabel(side)}</button>{room.isOwner && occupiedByOther && <button className="remove" disabled={pending} onClick={() => send('room-remove-seat', { side })}>移除成员</button>}</article>
        })}</div>

...
function LanLobby({ nickname, onNickname }: { nickname: string; onNickname: (value: string) => void }) {
  ...
  const roomItems = [
    ...rooms.map(room => ({ room, recent: recent.some(item => item.id === room.id) })),
    ...historyRooms.map(room => ({ room, recent: recent.some(item => item.id === room.id) })),
  ].filter(item => roomFilter === 'all' || roomFilter === 'recent' ? roomFilter !== 'recent' || item.recent : item.room.phase === roomFilter)
  const recentOnly = roomFilter === 'recent' ? recent.filter(item => !roomIds.has(item.id)) : []
  return <div className="lan-shell"><header className="lan-header"><div><h1>象棋局域网对战</h1><p>同一局域网内创建房间、邀请棋友、聊天或观战</p></div><a href="?type=xiangqi">返回模式选择</a></header>
    ...
    {lobbyView === 'create' && <form className="lan-create card" onSubmit={event => { event.preventDefault(); void create() }}><div className="lan-create-heading"><div><h2>创建新房间</h2><p>填写信息并选择玩法，创建后进入独立准备室。</p></div><span>1 分钟内最多创建 10 个房间</span></div><div className="lan-form-grid"><label>你的昵称<input value={nickname} maxLength={20} onChange={e => onNickname(e.target.value)} placeholder="例如：棋友小李" autoComplete="nickname" /></label><label>房间名称<input value={name} maxLength={60} onChange={e => setName(e.target.value)} placeholder="例如：周末切磋" /></label></div><fieldset className="lan-variant-picker"><legend>选择玩法</legend><button type="button" className={variant === 'xiangqi' ? 'active' : ''} onClick={() => setVariant('xiangqi')}><strong>普通象棋</strong><span>标准规则，支持协商悔棋</span><i>{variant === 'xiangqi' ? '已选择' : '选择'}</i></button><button type="button" className={variant === 'jieqi' ? 'active' : ''} onClick={() => setVariant('jieqi')}><strong>揭棋</strong><span>暗子随机分配，移动后揭晓身份</span><i>{variant === 'jieqi' ? '已选择' : '选择'}</i></button></fieldset><button className="lan-create-submit" type="submit" disabled={!nickname.trim() || creating}>{creating ? '正在创建并进入…' : `创建${variant === 'jieqi' ? '揭棋' : '普通象棋'}房间`}</button></form>}
    {error && <div className="lan-error"><ProductState kind="error" title="大厅暂时不可用" description={error} /></div>}
    {lobbyView === 'find' && <section className="lan-lobby card"><div className="lan-lobby-heading"><div><h2>房间列表</h2><p>公开、最近进入和最近结束统一展示</p></div><div className="lan-room-filters">{(['all', 'waiting', 'playing', 'finished', 'recent'] as const).map(filter => <button className={roomFilter === filter ? 'active' : ''} key={filter} onClick={() => setRoomFilter(filter)}>{{ all: '全部', waiting: '可加入', playing: '进行中', finished: '已结束', recent: '最近进入' }[filter]}</button>)}</div></div>{roomItems.length === 0 && recentOnly.length === 0 ? <ProductState title="当前筛选下没有房间" description={roomFilter === 'waiting' ? '可以创建一个房间并邀请棋友加入。' : '切换筛选条件看看其他房间。'} /> : <div className="lan-room-list">{roomItems.map(({ room, recent: isRecent }) => <LanRoomCard key={room.id} name={room.name} meta={`${room.variant === 'jieqi' ? '揭棋' : '普通象棋'} · ${room.phase === 'waiting' ? '可加入' : room.phase === 'playing' ? '进行中' : formatResult(room.status, room.statusReason)}${isRecent ? ' · 最近进入' : ''}`} details={`红：${room.red || '空席'}　黑：${room.black || '空席'}　${room.phase === 'finished' ? `共 ${room.moveCount} 手` : `观众 ${room.spectatorCount}`}`} onOpen={() => enter(room.id)} />)}{recentOnly.map(item => <LanRoomCard key={item.id} name={item.name} meta={`${item.variant === 'jieqi' ? '揭棋' : '普通象棋'} · 最近进入`} details={item.role === 'red' ? '红方席位' : item.role === 'black' ? '黑方席位' : item.role === 'owner' ? '房主' : '观众'} onOpen={() => enter(item.id)} />)}</div>}</section>}
  </div>
}
...
function LanRoom({ roomId, nickname, onNickname }: { roomId: string; nickname: string; onNickname: (value: string) => void }) {
  ...
  if (room.phase === 'waiting') return <div className="lan-shell lan-room-shell lan-ready-shell">
    <header className="lan-header"><div><h1>{room.name}</h1><p>{room.variant === 'jieqi' ? '揭棋' : '普通象棋'} · 准备室 · {connected ? '已连接' : '重连中'}</p></div><a href="?lan=1">返回大厅</a></header>
    {error && <div className="lan-error">{error}</div>}
    <main className="lan-ready-layout">
      <section className="lan-ready-room card">
        <div className="lan-ready-heading"><div><small>{incomingInvite && !color ? '棋友邀请' : '等待开局'}</small><h2>{incomingInvite && !color ? '确认昵称与席位后加入' : '双方选择席位并准备'}</h2><p>{room.variant === 'jieqi' ? '揭棋：暗子移动后揭晓，暗吃身份仅捕获方可见。' : '普通象棋：红方先行，支持协商悔棋与和棋。'}</p></div><span>观众 {room.spectatorCount} 人</span></div>
        ...
        <div className="lan-ready-seats">{(['red', 'black'] as const).map(side => {
          const seat = room.seats[side], current = color === side, occupiedByOther = Boolean(seat && !current)
          const disabled = pending || Boolean(room.pendingSwapBy) || (!color && occupiedByOther) || (!color && !seat && !nickname.trim())
          return <LanReadySeat key={side} side={side} mark={side === 'red' ? '红' : '黑'} title={side === 'red' ? '红方 · 先行' : '黑方 · 后手'} status={seat ? `${seat.nickname} · ${seat.online ? '在线' : '离线'} · ${seat.ready ? '已准备' : '未准备'}` : '席位空缺'} current={current} disabled={disabled} actionLabel={boardSeatLabel(side)} onAction={() => actOnBoardSeat(side)} onRemove={room.isOwner && occupiedByOther ? () => send('room-remove-seat', { side }) : undefined} removeDisabled={pending} />
        })}</div>
        {room.pendingSwapBy === color && <div className="lan-request lan-proposal-bar">换边申请等待对方处理（{proposalCountdown(room.pendingSwapDeadline)} 秒）<button disabled={pending} onClick={() => send('room-proposal-cancel', { kind: 'swap' })}>撤回</button></div>}
  ...
  return <div className="lan-shell lan-room-shell">
    <header className="lan-header"><div><h1>{room.name}</h1><p>{room.variant === 'jieqi' ? '揭棋' : '普通象棋'} · {room.phase === 'playing' ? '进行中' : '已结束'} · {connected ? '已连接' : '重连中'}</p></div><a href="?lan=1">返回大厅</a></header>
    {error && <div className="lan-error">{error}</div>}
    ...
    <div className="lan-game">
      ...
      <aside className="lan-panel lan-room-playing card"><h2>{color ? `你是${color === 'red' ? '红方' : '黑方'}` : '观战中'}</h2><p>{room.status === 'playing' ? `${room.turn === 'red' ? '红方' : '黑方'}走棋` : formatResult(room.status, room.statusReason)}</p><p>红：{room.seats.red?.nickname || '-'} {room.seats.red?.online ? '●' : '○'}<br/>黑：{room.seats.black?.nickname || '-'} {room.seats.black?.online ? '●' : '○'}</p>
        ...
        {room.pendingUndoBy === color && <div className="lan-request lan-proposal-bar">悔棋申请等待中（{proposalCountdown(room.pendingUndoDeadline)} 秒）<button disabled={pending} onClick={() => send('room-proposal-cancel', { kind: 'undo' })}>撤回</button></div>}
...
export function LanChat({ messages, connected, isOwner, settings, error, send, remove, mute, updateSettings, sideLabels = { red: '红方', black: '黑方' } }: { messages: LanChatMessage[]; connected: boolean; isOwner: boolean; settings: import('./types').LanChatSettings; error: string; send: (content: string) => boolean; remove: (messageId: string) => boolean; mute: (authorId: string, muted: boolean) => boolean; updateSettings: (everyoneMuted: boolean, words: string[]) => boolean; sideLabels?: { red: string; black: string } }) {
...
function Request({ text, pending, accept, reject }: { text: string; pending: boolean; accept: () => void; reject: () => void }) { return <div className="lan-request lan-proposal-bar">{text}<button disabled={pending} onClick={accept}>同意</button><button disabled={pending} onClick={reject}>拒绝</button></div> }

...
export async function listLanRooms(game: 'xiangqi' | 'gomoku' = 'xiangqi') {
  return (await json<{ rooms: LanRoomSummary[] }>(`/api/rooms/lobby?game=${game}`)).rooms
}
export async function listLanRoomHistory(game: 'xiangqi' | 'gomoku' = 'xiangqi') {
  return (await json<{ rooms: LanRoomSummary[] }>(`/api/rooms/history?game=${game}`)).rooms
}
export async function createLanRoom(name: string, variant: 'xiangqi' | 'jieqi') {
  return json<{
    ...
    ownerToken: string
    inviteToken: string
  }>('/api/rooms', { method: 'POST', body: JSON.stringify({ name, variant }) })
}

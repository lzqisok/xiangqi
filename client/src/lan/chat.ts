...
export function getLanChatContentLength(content: string) { return Array.from(content).length }
export function getLanChatLineCount(content: string) { return content.replace(/\r\n?/g, '\n').split('\n').length }

export function parseLanRoomSensitiveWords(content: string) {
  const words: string[] = [], seen = new Set<string>()
  for (const item of content.split(/[,，、\n]/)) {
    const word = item.replace(/[\u0000-\u001F\u007F]/g, '').replace(/\s+/g, ' ').trim()

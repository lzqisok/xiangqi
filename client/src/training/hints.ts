...
export function getEndgameTrainingHint(
  ...
  try {
    ...
    if (level === 1)
      return `方向提示：下一手重点关注${piece.color === 'red' ? '红方' : '黑方'}${pieceName}的主动走法。`
...
export function recordTrainingHintLevel(
  ...
  if (
    history.some(
      (item) =>
        item.moveIndex === moveIndex && item.level === level && item.positionKey === positionKey,
    )
  )
    return history
...
export function getEndgameTrainingFeedback(
  endgame: EndgameDefinition | null,
  records: MoveRecord[],
  status: string,
  ...
  if (solution.length > 0 && records.length > 0) {
    ...
    const firstMismatch = played.findIndex(
      (uci, index) => solution[index] && solution[index] !== uci,
    )
    if (firstMismatch >= 0) {
  ...
  if (
    endgame.target === 'survive' &&
    endgame.maxMoves &&
    records.length >= endgame.maxMoves &&
    status === 'playing'
  ) {
    return `目标完成：已坚持 ${endgame.maxMoves} 手。`
  }
  if (
    (endgame.target === 'red-win' || endgame.target === 'black-win') &&
    endgame.maxMoves &&
    records.length >= endgame.maxMoves &&
    status === 'playing'
  ) {

...
function isCompactMove(value: unknown): boolean {
  ...
  return (
    Boolean(move) &&
    UCI_MOVE.test(move!.u as string) &&
    (move!.q === undefined || (typeof move!.q === 'string' && move!.q.length <= 200)) &&
    (move!.e === undefined ||
      (typeof move!.e === 'number' && Number.isFinite(move!.e) && move!.e >= 0)) &&
...
function isCompactTree(value: unknown): value is StoredGameState['t'] {
  ...
  if (
    entries.length < 1 ||
    entries.length > MAX_VARIATION_NODES ||
    !nodes[tree.r] ||
    !nodes[tree.c]
  )
    return false
  for (const [id, rawNode] of entries) {
    ...
    if (
      ...
      !node.c.every((child) => typeof child === 'string' && Boolean(nodes[child]))
    )
      return false
    const isRoot = id === tree.r
    if (
      isRoot
        ? node.p !== null || node.v !== undefined
        : typeof node.p !== 'string' || !nodes[node.p] || !isCompactMove(node.v)
    )
      return false
    if (node.x !== undefined && (typeof node.x !== 'string' || !node.c.includes(node.x)))
      return false
  }
...
export function isGameConfig(value: unknown): value is GameConfig {
  ...
  return (
    Boolean(item) &&
    DIFFICULTIES.has(item!.difficulty as string) &&
    (item!.playerSide === 'red' || item!.playerSide === 'black') &&
    DIFFICULTIES.has(item!.aiRedDifficulty as string) &&
    DIFFICULTIES.has(item!.aiBlackDifficulty as string)
  )
}

export function isStoredGameState(value: unknown, mode: LiveGameMode): value is StoredGameState {
  const state = object(value)
  if (!state || typeof state.f !== 'string' || state.f.length > 300 || !isCompactTree(state.t))
    return false
  if (
    mode === 'jieqi'
      ? state.f !== JIEQI_INITIAL_FEN || !isJieqiLayout(state.j)
      : !validateFenPosition(state.f).ok || state.j !== undefined
  )
    return false
  return (
    STATUSES.has(state.s as GameStatus) &&
    (state.g === undefined || STATUS_REASONS.has(state.g as string))
  )
}
...
export function isGameDocument(value: unknown): value is GameDocument {
  ...
  if (
    ...
    !game.name.trim() ||
    game.name.length > 100
  )
    return false
  if (
    !isLiveGameMode(game.mode) ||
    !isGameConfig(game.config) ||
    !isStoredGameState(game.state, game.mode)
  )
    return false
  return (
...
export function isGameSummary(value: unknown): value is GameSummary {
  ...
  return (
    ...
    isLiveGameMode(game!.mode) &&
    isGameConfig(game!.config) &&
    STATUSES.has(game!.status as GameStatus) &&

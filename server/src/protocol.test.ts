...
test('parseClientMessage validates game lease messages', () => {
  ...
  const claim = parseClientMessage(
    JSON.stringify({ type: 'claim-game', requestId: 'lease-1', gameId }),
  )
  ...
  assert.equal(
    parseClientMessage(
      JSON.stringify({ type: 'claim-game', requestId: 'lease-2', gameId: '../games.json' }),
    ).ok,
    false,
  )
})
...
test('parseClientMessage accepts extended reveal moves for the Jieqi engine only', () => {
  ...
  const standard = parseClientMessage(
    JSON.stringify({
      type: 'move',
...
test('parseClientMessage validates Jieqi rank widths, kings, and reserve inventory', () => {
  ...
  assert.equal(
    request(
      'xxxx1xxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w R2A2C2P5N2B2r2a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
  assert.equal(
    request(
      'xxxxkxxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w R3A1C2P5N2B2r2a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
  assert.equal(
    request(
      'xxxxkxxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w R2A2C2P4N2B2r2a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
  assert.equal(
    request(
      '1xxxkxxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w R2A2C2P5N2B2r1a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
  assert.equal(
    request(
      'xxxxkxxxx/9/1x5x1/x1x1x1x1x/9/9/X1X1X1X1X/1X5X1/9/XXXXKXXXX w R1R1A2C2P5N2B2r2a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
  assert.equal(
    request(
      'xxxx1xxxx/9/1x5x1/x1x1x1x1x/3k5/5K3/X1X1X1X1X/1X5X1/9/XXXX1XXXX w R2A2C2P5N2B2r2a2c2p5n2b2 0 1',
    ).ok,
    false,
  )
})
...
test('parseClientMessage accepts candidate requests with bounded count', () => {
  ...
  const fractional = parseClientMessage(
    JSON.stringify({
      type: 'candidates',
      requestId: 'c3',
      fen: VALID_FEN,
...
test('parseClientMessage accepts bounded review requests and requires an id', () => {
  ...
  const missingId = parseClientMessage(
    JSON.stringify({ type: 'review', fen: VALID_FEN, moves: [] }),
  )
...
test('parseClientMessage rejects invalid JSON, missing requestId, and bad FEN', () => {
  ...
  const badFen = parseClientMessage(
    JSON.stringify({ type: 'analyze', requestId: 'a1', fen: 'bad fen' }),
  )
...
test('parseClientMessage rejects invalid search limits', () => {
  ...
  const badDepth = parseClientMessage(
    JSON.stringify({
      type: 'move',
  ...
  const badTime = parseClientMessage(
    JSON.stringify({
      type: 'candidates',
  ...
  if (!badTime.ok)
    assert.equal(badTime.error, 'searchTimeMs must be an integer between 500 and 10000')
})
...
test('parseClientMessage validates engine runtime options', () => {
  ...
  const badThreads = parseClientMessage(
    JSON.stringify({
      type: 'init',
      difficulty: 'medium',
      engineThreads: 12,
  ...
  if (!badThreads.ok)
    assert.equal(badThreads.error, 'engineThreads must be "auto" or an integer between 1 and 8')

  const badHash = parseClientMessage(
    JSON.stringify({
      type: 'init',
      difficulty: 'medium',
      engineHashMb: 8,
...
test('parseClientMessage rejects empty requestId and malformed UCI moves', () => {
  const emptyRequestId = parseClientMessage(
    JSON.stringify({ type: 'move', requestId: '', fen: VALID_FEN, moves: [] }),
  )
  ...
  const badMoves = parseClientMessage(
    JSON.stringify({
      type: 'move',
      requestId: 'move-bad',
      fen: VALID_FEN,
...
test('parseClientMessage rejects UCI move lists that cannot legally replay from FEN', () => {
  const ownPieceCapture = parseClientMessage(
    JSON.stringify({
      type: 'move',
...
test('parseClientMessage rejects move lists without a FEN replay base', () => {
  const missingFen = parseClientMessage(
    JSON.stringify({
      type: 'move',
...
test('isStaleEngineResponse guards request id, kind, and move history', () => {
  ...
  assert.equal(
    isStaleEngineResponse(pending, { requestId: 'move-1', requestKind: 'move' }, 'h2e2'),
    false,
  )
  assert.equal(
    isStaleEngineResponse(pending, { requestId: 'move-2', requestKind: 'move' }, 'h2e2'),
    true,
  )
  assert.equal(
    isStaleEngineResponse(pending, { requestId: 'move-1', requestKind: 'hint' }, 'h2e2'),
    true,
  )
  assert.equal(
    isStaleEngineResponse(pending, { requestId: 'move-1', requestKind: 'move' }, 'h2e2 h9g7'),
    true,
  )
  assert.equal(
    isStaleEngineResponse(null, { requestId: 'move-1', requestKind: 'move' }, 'h2e2'),
    true,
  )
})

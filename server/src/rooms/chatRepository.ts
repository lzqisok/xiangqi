...
function validMessage(value: unknown): value is RoomChatMessage {
  ...
  return (
    ...
    message.nickname.length <= 20 &&
    ROLES.has(message.role) &&
    typeof message.isOwner === 'boolean' &&
    typeof message.content === 'string' &&
    message.content.trim().length > 0 &&
    Array.from(message.content).length <= MAX_ROOM_CHAT_CONTENT_LENGTH &&
...
function validChat(value: unknown, roomId: string): value is StoredRoomChat {
  ...
  if (
    ...
    chat.roomId !== roomId ||
    !Number.isInteger(chat.sequence) ||
    chat.sequence < 0
  )
    return false
  if (
    !Array.isArray(chat.messages) ||
    chat.messages.length > MAX_ROOM_CHAT_MESSAGES ||
    !chat.messages.every(validMessage)
  )
    return false
  if (
    typeof chat.everyoneMuted !== 'boolean' ||
    !Array.isArray(chat.mutedAuthorIds) ||
    chat.mutedAuthorIds.length > 100 ||
    !chat.mutedAuthorIds.every((id) => AUTHOR_ID.test(id))
  )
    return false
  try {
    if (
      JSON.stringify(normalizeRoomSensitiveWords(chat.roomSensitiveWords)) !==
      JSON.stringify(chat.roomSensitiveWords)
    )
      return false
  } catch {
    return false
  ...
  return (
    chat.messages.every(
      (message, index) => index === 0 || message.sequence > chat.messages[index - 1].sequence,
...
export class RoomChatRepository {
  ...
  async init(roomIds: Set<string>) {
    ...
    for (const file of await readdir(this.directory)) {
      ...
      const chat =
        (await this.read(target, match[1])) || (await this.read(`${target}.bak`, match[1]))
      if (chat) this.chats.set(chat.roomId, chat)
    }
  }

  list(roomId: string) {
    return structuredClone(this.chats.get(roomId)?.messages || [])
  }

  settings(roomId: string) {
    const chat = this.chats.get(roomId)
    return structuredClone({
      everyoneMuted: chat?.everyoneMuted || false,
      mutedAuthorIds: chat?.mutedAuthorIds || [],
      roomSensitiveWords: chat?.roomSensitiveWords || [],
    })
  }
  ...
  async append(roomId: string, message: Omit<RoomChatMessage, 'sequence'>) {
    ...
    await this.mutate(roomId, async () => {
      ...
      const next: StoredRoomChat = {
        ...
        messages: [...current.messages, appended].slice(-MAX_ROOM_CHAT_MESSAGES),
      }
  ...
  async updateSettings(
    roomId: string,
    settings: { everyoneMuted?: boolean; mutedAuthorIds?: string[]; roomSensitiveWords?: string[] },
  ) {
    await this.mutate(roomId, async () => {
      const current = this.chats.get(roomId) || this.empty(roomId)
      const next: StoredRoomChat = {
        ...
        mutedAuthorIds: settings.mutedAuthorIds
          ? [...new Set(settings.mutedAuthorIds)].slice(0, 100)
          : current.mutedAuthorIds,
        roomSensitiveWords: settings.roomSensitiveWords
          ? normalizeRoomSensitiveWords(settings.roomSensitiveWords)
          : current.roomSensitiveWords,
      }
      if (!next.mutedAuthorIds.every((id) => AUTHOR_ID.test(id)))
        throw new Error('禁言成员标识无效')
      await this.write(next)
      this.chats.set(roomId, structuredClone(next))
    })
  ...
  private target(roomId: string) {
    return path.join(this.directory, `${roomId}.chat.json`)
  }
  private empty(roomId: string): StoredRoomChat {
    return {
      schemaVersion: 1,
      roomId,
      sequence: 0,
  ...
  private async write(chat: StoredRoomChat) {
    ...
    try {
      ...
      try {
        await handle.writeFile(JSON.stringify(chat), 'utf8')
        await handle.sync()
      } finally {
        await handle.close()
      }

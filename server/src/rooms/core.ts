...
import { RoomBoard, RoomColor, RoomMove, RoomPiece, RoomStatus, RoomStatusReason, RoomVariant } from './types.js'

export const ROOM_INITIAL_FEN = 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1'
const TYPES: RoomPiece['type'][] = ['r', 'r', 'a', 'a', 'b', 'b', 'n', 'n', 'c', 'c', 'p', 'p', 'p', 'p', 'p']
const MAP: Record<string, RoomPiece> = {}
for (const [char, type] of Object.entries({ k: 'k', a: 'a', b: 'b', n: 'n', r: 'r', c: 'c', p: 'p' } as const)) {
  MAP[char] = { type, color: 'black' }
  MAP[char.toUpperCase()] = { type, color: 'red' }
}

function clone(board: RoomBoard): RoomBoard { return board.map(row => row.map(piece => piece ? { ...piece } : null)) }
function inside(row: number, col: number) { return row >= 0 && row < 10 && col >= 0 && col < 9 }
function pos(text: string) { return { col: text.charCodeAt(0) - 97, row: 9 - Number(text[1]) } }
...
export function createRoomInitialState(variant: RoomVariant): { board: RoomBoard; layout?: string } {
  ...
  const hidden = board.map(row => row.map(piece => {
...
function findKing(board: RoomBoard, color: RoomColor) {
  for (let row = 0; row < 10; row++) for (let col = 0; col < 9; col++) if (board[row][col]?.type === 'k' && board[row][col]?.color === color) return { row, col }
  return null
}
...
function pseudo(board: RoomBoard, from: { row: number; col: number }, variant: RoomVariant) {
  ...
  const add = (r: number, c: number) => { if (inside(r, c) && !own(r, c)) result.push({ row: r, col: c }) }
  ...
  if (type === 'k') {
    ...
    if (other?.col === from.col) {
      let blocked = false
      for (let row = Math.min(from.row, other.row) + 1; row < Math.max(from.row, other.row); row++) if (board[row][from.col]) blocked = true
      if (!blocked) add(other.row, other.col)
    }
  } else if (type === 'a') {
    for (const [dr, dc] of [[1, 1], [1, -1], [-1, 1], [-1, -1]]) {
      const row = from.row + dr, col = from.col + dc
      if (variant === 'jieqi' && !piece.hidden || col >= 3 && col <= 5 && (red ? row >= 7 && row <= 9 : row >= 0 && row <= 2)) add(row, col)
    }
  } else if (type === 'b') {
    for (const [dr, dc] of [[2, 2], [2, -2], [-2, 2], [-2, -2]]) {
      const row = from.row + dr, col = from.col + dc
      if ((variant === 'jieqi' && !piece.hidden || (red ? row >= 5 : row <= 4)) && !board[from.row + dr / 2]?.[from.col + dc / 2]) add(row, col)
    }
  } else if (type === 'n') {
    ...
    for (const [dr, dc] of [[0,1],[0,-1],[1,0],[-1,0]]) {
      let row = from.row + dr, col = from.col + dc, screen = false
      while (inside(row, col)) {
        if (board[row][col]) {
          if (type === 'r') { add(row, col); break }
          if (!screen) screen = true
          else { add(row, col); break }
    ...
    if (red ? from.row <= 4 : from.row >= 5) { add(from.row, from.col - 1); add(from.row, from.col + 1) }
  }
  return result
}

function apply(board: RoomBoard, from: { row: number; col: number }, to: { row: number; col: number }) {
  const next = clone(board), captured = next[to.row][to.col], moving = next[from.row][from.col]
  next[to.row][to.col] = moving
  next[from.row][from.col] = null
  let revealed: RoomPiece['type'] | undefined
  if (moving?.hidden) { moving.hidden = false; delete moving.darkType; revealed = moving.type }
...
function inCheck(board: RoomBoard, color: RoomColor, variant: RoomVariant): boolean {
  ...
  for (let row = 0; row < 10; row++) for (let col = 0; col < 9; col++) {
    ...
    if (piece?.color !== color && !(variant === 'jieqi' && piece?.hidden) && pseudo(board, { row, col }, variant).some(move => move.row === king.row && move.col === king.col)) return true
  }
  return false
}

export function legalRoomMoves(board: RoomBoard, from: { row: number; col: number }, variant: RoomVariant) {
  const piece = board[from.row]?.[from.col]
  if (!piece) return []
  return pseudo(board, from, variant).filter(to => !inCheck(apply(board, from, to).board, piece.color, variant))
}

function status(board: RoomBoard, turn: RoomColor, variant: RoomVariant): { status: RoomStatus; reason?: RoomStatusReason } {
  if (!findKing(board, turn)) return { status: turn === 'red' ? 'black-wins' : 'red-wins', reason: 'checkmate' }
  for (let row = 0; row < 10; row++) for (let col = 0; col < 9; col++) {
    if (board[row][col]?.color === turn && legalRoomMoves(board, { row, col }, variant).length) return { status: 'playing' }
  }
  return { status: turn === 'red' ? 'black-wins' : 'red-wins', reason: inCheck(board, turn, variant) ? 'checkmate' : 'stalemate' }
}

export function boardFen(board: RoomBoard, turn: RoomColor) {
  return `${board.map(row => { let text = '', empty = 0; for (const piece of row) { if (!piece) empty++; else { if (empty) text += empty; empty = 0; const char = piece.type; text += piece.color === 'red' ? char.toUpperCase() : char } } return text + (empty || '') }).join('/')} ${turn === 'red' ? 'w' : 'b'} - - 0 1`
}
...
function repetitionOccurrences(moves: RoomMove[], target: string) {
  ...
  for (const move of moves) {
    const from = pos(move.uci.slice(0, 2)), to = pos(move.uci.slice(2))
...
export function rebuildRoomBoard(variant: RoomVariant, layout: string | undefined, moves: RoomMove[]) {
  let board = variant === 'jieqi' ? boardFromLayout(layout || '') : parseRoomFen().board
  let turn: RoomColor = 'red'
  for (const move of moves) { const from = pos(move.uci.slice(0, 2)), to = pos(move.uci.slice(2)); board = apply(board, from, to).board; turn = turn === 'red' ? 'black' : 'red' }
  return { board, turn }
}
...
export function executeRoomMove(variant: RoomVariant, layout: string | undefined, moves: RoomMove[], uci: string, color: RoomColor) {
  ...
  if (detail.status === 'playing' && variant === 'xiangqi') {
    ...
    else if (nextMoves.slice(-120).length === 120 && nextMoves.slice(-120).every(item => !item.captured)) detail = { status: 'draw', reason: 'natural-limit' }
...
export function projectBoard(board: RoomBoard): RoomBoard {
  return board.map(row => row.map(piece => piece?.hidden ? { color: piece.color, type: piece.darkType || piece.type, darkType: piece.darkType, hidden: true } : piece ? { ...piece } : null))
}

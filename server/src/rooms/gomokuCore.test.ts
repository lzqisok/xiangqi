...
test('Gomoku room core validates turn, bounds, and occupied points', () => {
  ...
  assert.throws(
    () =>
      executeGomokuMove(rebuildGomokuRoom([first.move]), [first.move], 7, 7, 'black', 'freestyle'),
    /已有棋子/,
  )
})
...
test('Renju rejects an overline without persisting the forbidden move', () => {
  ...
  assert.throws(
    () => executeGomokuMove(rebuildGomokuRoom(moves), moves, 7, 3, 'red', 'renju'),
    /黑方禁手/,
  )
...
test('Gomoku rebuild rejects out-of-turn or mismatched move records', () => {
  assert.throws(
    () => rebuildGomokuRoom([{ uci: 'hh', color: 'black', row: 7, col: 7 }]),
    /记录无效/,
  )

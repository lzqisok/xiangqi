...
function chain(board: GomokuRoomBoard, row: number, col: number, color: RoomColor, dr: number, dc: number) {
  const points = [{ row, col }]
  for (let r = row + dr, c = col + dc; inside(r, c) && board[r][c] === color; r += dr, c += dc) points.push({ row: r, col: c })
  for (let r = row - dr, c = col - dc; inside(r, c) && board[r][c] === color; r -= dr, c -= dc) points.unshift({ row: r, col: c })
  return points
}

type LinePattern = Array<'x' | 'o' | '_' | '#'>
function key(row: number, col: number) { return row * SIZE + col }
function lineAt(board: GomokuRoomBoard, row: number, col: number, dr: number, dc: number) {
  const pattern: LinePattern = [], points: Array<{ row: number; col: number } | null> = []
  let center = 0
  for (let offset = -5; offset <= 5; offset++) {
    const r = row + dr * offset, c = col + dc * offset
    if (offset === 0) center = pattern.length
    if (!inside(r, c)) { pattern.push('#'); points.push(null); continue }
...
function canCreateExactFive(pattern: LinePattern, index: number, center: number) {
  ...
  const next = [...pattern]; next[index] = 'x'
...
function collectFourThreats(board: GomokuRoomBoard, row: number, col: number) {
  ...
  for (const [dr, dc] of DIRECTIONS) {
    ...
    const wins: number[] = []
    for (let index = 0; index < pattern.length; index++) if (Math.abs(index - center) <= 4 && canCreateExactFive(pattern, index, center)) wins.push(index)
    if (wins.length) total += isSingleOpenFour(pattern, wins) ? 1 : Math.min(2, wins.length)
  }
  return total
}
function hasOpenFourIncluding(board: GomokuRoomBoard, row: number, col: number, dr: number, dc: number, required: Array<{ row: number; col: number }>) {
  const { pattern, points } = lineAt(board, row, col, dr, dc)
  const requiredIndexes = required.map(point => points.findIndex(candidate => candidate?.row === point.row && candidate?.col === point.col))
  ...
  for (let index = 0; index <= pattern.length - 6; index++) {
    ...
    if (pattern.slice(index + 1, index + 5).every(cell => cell === 'x') && requiredIndexes.every(requiredIndex => requiredIndex >= index + 1 && requiredIndex <= index + 4)) return true
  }
  return false
}
function legalBlackExtension(board: GomokuRoomBoard, row: number, col: number, checking: Set<number>) {
...
function collectOpenThreeThreats(board: GomokuRoomBoard, row: number, col: number, checking: Set<number>) {
  let total = 0
  for (const [dr, dc] of DIRECTIONS) {
    ...
    for (let index = 0; index < pattern.length; index++) {
      ...
      if (open) { found = true; break }
    }
    if (found) total++
  }
  return total
}
function isForbiddenInternal(board: GomokuRoomBoard, row: number, col: number, checking: Set<number>) {
...
export function isGomokuForbiddenMove(board: GomokuRoomBoard, row: number, col: number) { return isForbiddenInternal(board, row, col, new Set()) }

function result(board: GomokuRoomBoard, row: number, col: number, color: RoomColor, renju: boolean): { status: RoomStatus; reason?: RoomStatusReason } {
  for (const [dr, dc] of DIRECTIONS) {
    const length = chain(board, row, col, color, dr, dc).length
    if (color === 'red' && renju ? length === 5 : length >= 5) return { status: color === 'red' ? 'red-wins' : 'black-wins', reason: 'five' }
  }
  return board.every(line => line.every(Boolean)) ? { status: 'draw', reason: 'full-board' } : { status: 'playing' }
}
...
export function rebuildGomokuRoom(moves: RoomMove[]): RebuiltGomokuRoom {
  ...
  for (const [index, move] of moves.entries()) {
    ...
    const expectedCode = Number.isInteger(move.row) && Number.isInteger(move.col) ? `${String.fromCharCode(97 + move.col!)}${String.fromCharCode(97 + move.row!)}` : ''
    if (move.color !== expectedColor || move.uci !== expectedCode || !Number.isInteger(move.row) || !Number.isInteger(move.col) || !inside(move.row!, move.col!) || board[move.row!][move.col!]) throw new Error('五子棋记录无效')
    board[move.row!][move.col!] = move.color
  }
  return { board, turn: moves.length % 2 === 0 ? 'red' : 'black' }
}

export function executeGomokuMove(rebuilt: RebuiltGomokuRoom, moves: RoomMove[], row: number, col: number, color: RoomColor, rule: 'freestyle' | 'renju') {
  ...
  board[row][col] = color
  if (rule === 'renju' && color === 'red' && isGomokuForbiddenMove(board, row, col)) throw new Error('该位置为黑方禁手')
  const move: RoomMove = { uci: `${String.fromCharCode(97 + col)}${String.fromCharCode(97 + row)}`, color, row, col, notation: `${String.fromCharCode(65 + col)}${row + 1}` }
  return { board, turn: color === 'red' ? 'black' as const : 'red' as const, move, detail: result(board, row, col, color, rule === 'renju'), moves: [...moves, move] }
}

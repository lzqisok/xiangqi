...
export function createRoomRouter(manager: RoomManager): Router {
  ...
  router.get('/lobby', (req, res) => res.json({ rooms: manager.lobby(req.query.game === 'gomoku' ? 'gomoku' : req.query.game === 'xiangqi' ? 'xiangqi' : undefined) }))
  router.get('/history', (req, res) => res.json({ rooms: manager.history(Number(req.query.limit) || 20, req.query.game === 'gomoku' ? 'gomoku' : req.query.game === 'xiangqi' ? 'xiangqi' : undefined) }))
  router.post('/', async (req, res) => {
    try {
      ...
      if (req.body?.variant !== 'xiangqi' && req.body?.variant !== 'jieqi' && req.body?.variant !== 'gomoku') return res.status(400).json({ error: '玩法无效' })
      if (req.body?.variant === 'gomoku' && req.body?.gomokuRule !== 'freestyle' && req.body?.gomokuRule !== 'renju') return res.status(400).json({ error: '五子棋规则无效' })
      const created = await manager.createRoom(String(req.body?.name || ''), req.body.variant, req.body.gomokuRule)
